...
export default function EnvironmentsPage() {
  ...
  const [modal,   setModal]   = useState(null) // null | 'create' | env object
  const [form,    setForm]    = useState({ name: '', description: '', console_url: '' })
  ...
  function openEdit(env) { setForm({ name: env.name, description: env.description, console_url: env.console_url }); setModal(env); setError('') }
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      else await updateEnvironment(modal.id, form)
      setModal(null); load()
                ...
                <tr key={env.id} className="hover:bg-gray-50/50">
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2 justify-end">
                      <button onClick={() => openEdit(env)} className="text-xs text-gray-400 hover:text-indigo-600 transition-colors">Edit</button>
                      <button onClick={() => toggleActive(env)} className={`text-xs transition-colors ${env.is_active ? 'text-gray-400 hover:text-red-500' : 'text-gray-400 hover:text-emerald-600'}`}>
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h2 className="text-base font-semibold text-gray-900">
                {modal === 'create' ? 'New Environment' : `Edit ${modal.name}`}
              </h2>
            ...
            <form onSubmit={handleSave} className="px-6 py-5 space-y-4">
              {[['Name *', 'name', 'text'], ['Description', 'description', 'text'], ['Console URL', 'console_url', 'url']].map(([label, field, type]) => (
                <div key={field}>
